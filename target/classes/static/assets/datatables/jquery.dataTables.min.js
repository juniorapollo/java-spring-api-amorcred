...
oPaginate:{sFirst:"First",sLast:"Last",sNext:"Próximo",sPrevious:"Anterior"},sEmptyTable:"No data available in table",sInfo:"Exibindo _START_ - _END_ de _TOTAL_ registros",sInfoEmpty:"Exibindo 0 - 0 de 0 registro",sInfoFiltered:"(total _MAX_ )",sInfoPostFix:"",sDecimal:"",sThousands:",",sLengthMenu:"Exibir _MENU_ registros ",sLoadingRecords:"Carregando...",sProcessing:"Processando...",sSearch:"",sSearchPlaceholder:"Procurar...",sUrl:"",sZeroRecords:"Nenhum registro encontrado"},oSearch:h.extend({},
...
//$.fn.dataTable.ext.search.push(function(settings, data, dataIndex){var min = document.getElementById('min').value * 1;var max = document.getElementById('max').value * 1;var version = data[0] == "-" ? 0 : data[0] * 1;if (min == "" && max == "") {return true;} else if (min == "" && version < max) {return true;} else if (min < version && "" == max) {return true;} else if (min <= version && version <= max) {return true;}return false;});$(document).ready(function() {var table = $('#datatable').DataTable();
...
$(document).ready(function () {
    $("#pesquisar").click(function () {
        $("#minDia").val(dia($("#dataMin").val()));
        $("#minDia").change();           
        $("#minMes").val(mes($("#dataMin").val()));
        $("#minMes").change();        
        $("#minAno").val(ano($("#dataMin").val()));
        $("#minAno").change(); 
        
        $("#maxDia").val(dia($("#dataMax").val()));
        $("#maxDia").change();        
        $("#maxMes").val(mes($("#dataMax").val()));
        $("#maxMes").change();       
        $("#maxAno").val(ano($("#dataMax").val()));
        $("#maxAno").change();  
        
        
        //GerarPeríodo
        var dataInicial = $("#dataMin").val();
        var dataFinal = $("#dataMax").val();
        var dataInicialFormatada=(dia(dataInicial)+"/"+mes(dataInicial)+"/"+ano(dataInicial));
        var dataFinalFormatada=(dia(dataFinal)+"/"+mes(dataFinal)+"/"+ano(dataFinal));
        
        if(dataInicial==dataFinal){
            $("#relDiaInicial").html("<p>Data : "+dataInicialFormatada+"</p>" );
      
        }else{
        $("#relDiaInicial").html("<p>Período: "+dataInicialFormatada +" à "+ dataFinalFormatada+"</p>" );
        }
    });    
});
//Converte a data do Input em DIa mes e ano 

function dia(data) {
    JSON.stringify(data);
    var res = data.substr(8, 2);
    return res;
}

function mes(data) {
    JSON.stringify(data);
    var res = data.substr(5, 2);
    return res;
}
function ano(data) {
    JSON.stringify(data);
